...
const Model = function () {
    ...
    const onAllSelected = async ( e ) => {
        ...
        if(e.target.checked){
            ...
            source = source.filter( item => !item.includes( 'hkgolden' ));
            this.props.setSource(source);}
    ...
    const onSubsourceSelected = async( subsource ) => {
        ...
        let source = this.props.source.filter( item => item.includes( 'hkgolden' ));
        source = source.sort();
    ...
    const filterSubsource = ( keyword) => {
        ...
        result.filterList = subsourceList.filter( item => item.includes( keyword ));
